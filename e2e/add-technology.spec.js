// @ts-check
// E2E: Добавление технологии через Django/API baseline

const { test, expect } = require('@playwright/test');
const { loginAsOwner } = require('./helpers/auth');
...
async function openAddTechnologyPanel(page) {
  ...
  await page.evaluate(() => {
    const button = document.getElementById('chooseAddTech');
    if (!button) {
      throw new Error('chooseAddTech button not found');
    }
    button.click();
  });
...
test.describe('Добавление технологии через API', () => {
