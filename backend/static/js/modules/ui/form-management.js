...
import { DOMCache } from '../core/dom-utils.js';
  ...
  function canManageTechnologies() {
    const moderationFlow = getModerationFlow();
    return Boolean(moderationFlow && typeof moderationFlow.canManageTechnologies === 'function' && moderationFlow.canManageTechnologies());
  }
    ...
    return (Array.isArray(technologies) ? technologies : []).reduce((acc, technology) => {
      ...
      companies
        ...
        .forEach(company => {
          ...
          acc[company].push(technology);
        });
      return acc;
    }, {});
  }

  function getEnterpriseIdsForNames(names) {
    const enterprises = (window.StateManager && typeof window.StateManager.get === 'function')
      ? (window.StateManager.get('enterprisesList') || [])
      : [];
    return (Array.isArray(names) ? names : [])
      .map(name => {
        const normalized = String(name || '').trim();
        const found = enterprises.find(item => item && String(item.name || item).trim() === normalized);
  ...
  async function handleEditTechFormSubmit(e) {
    ...
    const companies = isHoldingWide ? [] : (Array.isArray(companiesValE) ? companiesValE : []);
  ...
  function initAddBlockFormHandler() {
    ...
    if (addBlockForm) {
      addBlockForm.onsubmit = async (e) => {
        e.preventDefault();
        const proposalOnlyMode = isProposalOnlyMode();
        if (!canManageTechnologies() && !proposalOnlyMode) {
          DataLoader.showNotification('Недостаточно прав для сохранения функционального блока', false);
          return;
        }
        ...
        if (proposalOnlyMode) {
          const moderationFlow = getModerationFlow();
          if (!moderationFlow || typeof moderationFlow.createProposal !== 'function') {
            DataLoader.showNotification('Модуль предложений недоступен', false);
            return;
          }
          const payload = {
            referenceType: 'functional_block',
            operation: hasEnterprise && hasBlockFromList && !hasBlockName ? 'map_existing' : 'create_block',
            blockName: blockName,
            existingBlocks: selectedBlocks,
            enterpriseNames: selectedEnterprises,
            enterpriseIds: getEnterpriseIdsForNames(selectedEnterprises),
            directionName: sectorName
          };
          try {
            await moderationFlow.createProposal('create', {
              payload,
              comment: hasEnterprise && hasBlockFromList && !hasBlockName
                ? 'Предложение на привязку функционального блока к предприятиям'
                : 'Предложение на добавление функционального блока'
            });
            if (typeof window.hideModal === 'function') {
              window.hideModal('addBlockPanel');
            }
          } catch (err) {
            DataLoader.showNotification((err && err.message) ? err.message : 'Не удалось отправить предложение по блоку', false);
          }
          return;
        }
