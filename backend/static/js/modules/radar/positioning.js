...
// ОБНОВЛЕНО (2026-01-29): Кеш для расчетов позиций технологий
  ...
  let savePositionCacheTimeout = null;
  ...
  function savePositionCache() {
    if (savePositionCacheTimeout) {
      clearTimeout(savePositionCacheTimeout);
      savePositionCacheTimeout = null;
    }
    if (RADAR_DEBUG && Logger && typeof Logger.debug === 'function') {
      Logger.debug(`[Positioning] Кеш позиций обновлен в памяти (${positionCache.size} записей)`);
    }
    return;
    try {
  ...
  function loadPositionCache() {
    return;
    try {
      const hadLegacyCache = localStorage.getItem(CACHE_STORAGE_KEY) !== null
        || localStorage.getItem(CACHE_STORAGE_VERSION_KEY) !== null;
      localStorage.removeItem(CACHE_STORAGE_KEY);
      localStorage.removeItem(CACHE_STORAGE_VERSION_KEY);
      if (hadLegacyCache && Logger && typeof Logger.debug === 'function') {
        Logger.debug('[Positioning] Legacy-кеш позиций удален из localStorage');
      }
    } catch (e) {
      if (Logger && typeof Logger.warn === 'function') {
        Logger.warn('[Positioning] Не удалось очистить legacy-кеш позиций из localStorage', e);
      }
    }
    return;
    try {
