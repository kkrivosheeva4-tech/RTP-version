...
'use strict';
  ...
  const RADAR_DEBUG = typeof window !== 'undefined'
    && new URLSearchParams(window.location.search).get('debugRadar') === 'true';
  ...
  function createBlip(tech, pos, quadrant, config) {
    ...
    el.addEventListener('click', (e) => {
      e.stopPropagation();
      if (RADAR_DEBUG && Logger) Logger.log('=== Клик на blip ===', {
        id: el.dataset.id,
      ...
      try {
        ...
        if (RADAR_DEBUG && Logger) Logger.log('blip click: технология найдена', {
        ...
        if (RADAR_DEBUG && Logger) Logger.log('blip click: showDetail доступна', {
          available: !!showDetailFn,
          type: typeof window.showDetail
        });

        if (t && showDetailFn) {
          if (RADAR_DEBUG && Logger) Logger.log('blip click: вызываем showDetail');
          showDetailFn(t, 'blip', blipQuadrant);
          if (RADAR_DEBUG && Logger) Logger.log('blip click: showDetail вызвана');
        } else {
          if (!t) {
  ...
  function renderRadar(data, config) {
    ...
    if (RADAR_DEBUG && Logger) Logger.debug('renderRadar: start — input data length:', Array.isArray(techData) ? techData.length : 0);
    ...
    if (RADAR_DEBUG && Logger) Logger.debug('renderRadar: start — valid techs:', validTechs.length);
    ...
    validTechs.forEach((t) => {
      ...
      if (isFullyImplemented) {
        ...
        if (!showImplemented) {
          if (RADAR_DEBUG && Logger && typeof Logger.debug === 'function') {
    ...
    if (RADAR_DEBUG && Logger) Logger.debug('renderRadar: after mapping — renderData entries:', renderData.length);
    ...
    renderData.forEach((entry) => {
      if (RADAR_DEBUG && Logger) Logger.debug('renderRadar: rendering blip', {
        id: entry.id,
        name: entry.name,
