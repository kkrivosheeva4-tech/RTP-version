...
function calculateColumnWidths(selectedFieldsList, previewData, companyFilterForDisplay) {
  ...
  return selectedFieldsList.map((field) => {
    ...
    (previewData || []).forEach((tech) => {
      const value =
        typeof window.getFieldValue === 'function'
...
function determinePageOrientation(minColWidths, selectedFieldsList) {
  ...
  const totalMinWidth =
    minColWidths.reduce((sum, w) => sum + w, 0) + (selectedFieldsList.length - 1) * cellPaddingMm;
...
async function generatePdf(sourceList, enterpriseName, selectedFields, companyFilterForDisplay) {
  const JsPdfCtor =
    typeof window !== 'undefined' && window.jspdf && typeof window.jspdf.jsPDF === 'function'
      ? window.jspdf.jsPDF
      : null;
  if (!JsPdfCtor) {
  ...
  const minColWidths = calculateColumnWidths(
    selectedFieldsList,
    previewData,
    companyFilterForDisplay
  );
  ...
  async function renderPagesToImages() {
    ...
    function drawHeader(cctx) {
      ...
      cctx.font = smallFont;
      cctx.fillText(
        `Дата формирования отчёта: ${nowStr}`,
        marginPx,
        y + Math.round((16 * pxPerMM) / 3.78)
      );
    ...
    selectedFieldsList.forEach((field, idx) => {
      const label =
        typeof window.getFieldLabel === 'function' ? window.getFieldLabel(field) : field;
    ...
    const headerHeight = Math.max(
      baseHeaderHeight,
      maxHeaderLines * Math.round((12 * pxPerMM) / 3.78) + cellPadding * 2
    );
      ...
      headerLines = headerLines.map((line) => {
        ...
        if (ctx.measureText(displayLine).width > availableWidth) {
          while (
            ...
            ctx.measureText(displayLine + '...').width > availableWidth
          ) {
    ...
    for (let i = 0; i < sourceList.length; i++) {
      ...
      const rowHeight = Math.max(
        headerHeight,
        maxLines * Math.round((12 * pxPerMM) / 3.78) + cellPadding * 2
      );
        ...
        selectedFieldsList.forEach((field, idx) => {
          ...
          headerLines = headerLines.map((line) => {
            ...
            if (cctx.measureText(displayLine).width > availableWidth) {
              while (
                displayLine.length > 0 &&
                cctx.measureText(displayLine + '...').width > availableWidth
              ) {
                displayLine = displayLine.slice(0, -1);
              }
              displayLine = displayLine + '...';
      ...
      selectedFieldsList.forEach((field, colIdx) => {
        ...
        const isNumeric =
          typeof window.isNumericField === 'function' ? window.isNumericField(field) : false;
        ...
        lines.forEach((line, lineIdx) => {
          ...
          if (ctx.measureText(displayLine).width > availableTextWidth) {
            while (
              displayLine.length > 0 &&
              ctx.measureText(displayLine + '...').width > availableTextWidth
            ) {
              displayLine = displayLine.slice(0, -1);
            }
            displayLine = displayLine + '...';
