...
class Tooltip {
  ...
  updatePosition() {
    ...
    if (
      (position === 'top' && top < margin) ||
      (position === 'bottom' && top + tooltipRect.height > window.innerHeight - margin)
    ) {
...
function initTooltip() {
  ...
  if (window.__tooltipInitialized) {
    if (window.Logger)
      window.Logger.debug('Tooltip уже инициализирован, пропускаем повторную инициализацию');
    return;
  }
...
function hasRoleCapability(capability) {
  ...
  return roleApi.hasCapability(capability, getCurrentRole());
}
...
function isAuthenticatedUi() {
  ...
  const roleApi = getRoleApi();
  if (roleApi && typeof roleApi.isKnownRole === 'function') {
    return roleApi.isKnownRole(getCurrentRole());
  }
  return false;
}

// Делегированный обработчик кликов для динамически добавляемых кнопок входа/выхода и ролей
// Примечание: logout обрабатывается только через onclick кнопок (common-ui/auth), чтобы избежать
// дублирования записей в adminAuditLogs при срабатывании и onclick, и делегированного обработчика
...
(function () {
  ...
  function renderInitialAnimation() {
    ...
    line1.setAttribute('y1', CENTER_Y);
    line1.setAttribute('x2', 950);
    line1.setAttribute('y2', CENTER_Y);
    const line2 = document.createElementNS(SVG_NS, 'line');
    line2.setAttribute('x1', CENTER_X);
    line2.setAttribute('y1', 50);
    line2.setAttribute('x2', CENTER_X);
    line2.setAttribute('y2', 950);
    ...
    highlight.setAttribute(
      'd',
      describeWedge(CENTER_X, CENTER_Y, maxRadius, -highlightTolerance, 0)
    );
...
function initDetailViewLinkVisibility() {
  ...
  function checkAndUpdateVisibility() {
    const isAuthorized = isAuthenticatedUi();
  ...
  window.addEventListener('rmk-auth-changed', checkAndUpdateVisibility);
