...
function clearLegacyAuthState() {
    ...
    try { sessionStorage.removeItem('authPasswordChangePending'); } catch (_) {}
}
...
function validateEmailInput(value) {
    ...
    if (!raw) return 'Введите email.';
    if (raw !== raw.trim()) return 'Email не должен содержать пробелы в начале или в конце.';
    if (/\s/.test(raw)) return 'Email не должен содержать пробелы.';
    const normalized = raw.toLowerCase();
    const emailRe = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
    if (!emailRe.test(normalized)) return 'Введите корректный email.';
    return '';
}
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            if (
                ...
                const userName = (me && me.username) || loginResult.data.username || email;
                if (window.AuthModule && typeof window.AuthModule.setAuthSession === 'function') {
                    window.AuthModule.setAuthSession(
                        { username: userName, email: email, role: userRole, is_2fa_enabled: true },
            ...
            if (loginResult.ok && loginResult.data && loginResult.data.requires_password_change && loginResult.data.session_id) {
                const userRole = loginResult.data.role || 'user';
                const userName = loginResult.data.username || '';
                try {
                    sessionStorage.setItem('authPasswordChangePending', JSON.stringify({
                        username: userName,
                        email: email,
                        role: userRole,
                    ...
                    if (window.Logger) window.Logger.warn('auth: не удалось сохранить authPasswordChangePending для API', err);
                }
