...
(function() {
  ...
  const TOUR_STEPS = [
    ...
    {
      id: 'prospects-chart',
      title: 'График Перспективных технологий',
      description: 'Нажмите на иконку графика в боковой панели, чтобы открыть модальное окно с графиком перспективных технологий. В этом окне вы можете просмотреть визуализацию технологий со статусом "Перспективные", выбрать конкретные предприятия для анализа и экспортировать график в PDF.',
      target: '#prospectsModal',
      position: 'center',
      showSkip: true,
      waitForElement: true,
      conditional: () => {
        // Показываем шаг, если модальное окно существует
        const prospectsModal = document.getElementById('prospectsModal');
        return prospectsModal !== null;
      },
      beforeShow: () => {
        // Открываем модальное окно графика перспективных технологий сразу, без задержки
        const prospectsModal = document.getElementById('prospectsModal');
        const toggleBtn = document.getElementById('toggleProspectsChartBtn');

        if (prospectsModal) {
          // Убеждаемся, что модальное окно видно поверх overlay
          prospectsModal.style.zIndex = '10005';
          prospectsModal.style.position = 'fixed';
          ...
          if (toggleBtn) {
            ...
            if (prospectsModal.classList.contains('hidden') || prospectsModal.hasAttribute('aria-hidden')) {
              prospectsModal.classList.remove('hidden');
              prospectsModal.setAttribute('aria-hidden', 'false');
              prospectsModal.style.display = 'block';
              prospectsModal.classList.add('open');

              // Вызываем функцию открытия модального окна, если она доступна
              if (typeof window.showModal === 'function') {
                window.showModal('prospectsModal');
              }
            }
          }
        }
      ...
      afterHide: () => {
        // Убираем подсветку кнопки "График" в боковой панели
        const chartIconBtn = document.getElementById('chartIconBtn');
        if (chartIconBtn) {
          chartIconBtn.classList.remove('onboarding-highlight');
        }
        ...
        if (prospectsModal) {
          if (typeof window.hideModal === 'function') {
            window.hideModal('prospectsModal');
          } else {
            prospectsModal.classList.add('hidden');
            prospectsModal.setAttribute('aria-hidden', 'true');
    ...
    },
    {
      ...
      description: 'Радар разделен на квадранты и кольца зрелости. Кликните на технологию, чтобы увидеть детальную информацию. Для детального просмотра сектора кликните на него - откроется увеличенный вид с модальным окном приоритетных технологий.',
      target: '#techRadar',
      position: 'center',
      showSkip: true
    },
    {
      ...
      description: 'Кликните на любой сектор радара, чтобы увеличить его и открыть модальное окно с приоритетными технологиями этого сектора. В модальном окне вы сможете просмотреть список технологий, отфильтровать их по статусу и найти нужную технологию.',
      target: '#techRadar',
      position: 'center',
      showSkip: true,
      beforeShow: () => {
    ...
    {
      id: 'priority-panel',
      title: 'Модальное окно приоритетов',
      description: 'При зуммировании сектора открывается модальное окно с приоритетными технологиями. Здесь отображаются все технологии выбранного сектора с возможностью их фильтрации и поиска.',
          ...
          if (quadrants.length > 0) {
            ...
            if (firstQuadrant && firstQuadrant.id) {
              ...
              requestAnimationFrame(() => {
                ...
                // Открываем панель приоритетов и заполняем список
                requestAnimationFrame(() => {
                  requestAnimationFrame(() => {
      ...
      afterHide: () => {
        ...
        // Панель и зум будут закрыты только после шага с приоритетностью технологии
    ...
    },
    {
      ...
      description: 'При клике на технологию в списке открывается панель с подробной информацией: описание, оценки, приоритет и примеры внедрений. Используйте эту панель для получения полной информации о технологии.',
      target: null,
      position: 'center',
      showSkip: true,
      conditional: () => {
        // Показываем шаг, если панель приоритетов существует
        const priorityPanel = document.getElementById('quadrantPriorityPanel');
        return priorityPanel !== null;
      },
      beforeShow: () => {
        ...
        // Убеждаемся, что панель приоритетов открыта и видна поверх overlay
        const priorityPanel = document.getElementById('quadrantPriorityPanel');
        if (priorityPanel) {
      ...
      afterHide: (stepIndex) => {
        ...
        // Закрываем панель приоритетов и убираем зум после шага
  ...
  function highlightElement(element, position = 'bottom') {
    ...
    const updateHighlight = () => {
      ...
      // Убеждаемся, что панель детальной информации и модальное окно приоритетов видимы поверх overlay
      const currentStep = TOUR_STEPS[currentStepIndex];
