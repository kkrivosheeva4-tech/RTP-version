...
(function () {
  ...
  function withAlpha(color, alpha) {
    ...
    if (c.indexOf('rgb(') === 0) {
      ...
      if (inner2.length >= 3)
        return 'rgba(' + inner2[0] + ', ' + inner2[1] + ', ' + inner2[2] + ', ' + alpha + ')';
    }
    if (c.indexOf('#') === 0) {
      var hex = c.slice(1);
      if (hex.length === 3)
    ...
    if (resolved && resolved.indexOf('rgb(') === 0) {
      ...
      if (inner3.length >= 3)
        return 'rgba(' + inner3[0] + ', ' + inner3[1] + ', ' + inner3[2] + ', ' + alpha + ')';
    }
  ...
  function getChartsThemeColors() {
    ...
    var text =
      (bodyStyles.getPropertyValue('--text') || bodyStyles.color || '').trim() ||
  ...
  function generateAuditData() {
    ...
    var values = order.map(function (a) {
      return counts[a] || 0;
    });
    return { labels: labels, values: values };
  }
  ...
  function applyChartsTheme() {
    ...
    if (usersChart) {
      ...
      if (
        usersChart.options &&
        usersChart.options.plugins &&
        usersChart.options.plugins.legend &&
        usersChart.options.plugins.legend.labels
      )
        usersChart.options.plugins.legend.labels.color = text;
      if (
        usersChart.options &&
        usersChart.options.scales &&
        usersChart.options.scales.x &&
        usersChart.options.scales.x.ticks
      )
        usersChart.options.scales.x.ticks.color = text;
      if (
        usersChart.options &&
        usersChart.options.scales &&
        usersChart.options.scales.y &&
        usersChart.options.scales.y.ticks
      )
        usersChart.options.scales.y.ticks.color = text;
      if (
        usersChart.options &&
        usersChart.options.scales &&
        usersChart.options.scales.x &&
        usersChart.options.scales.x.grid
      )
        usersChart.options.scales.x.grid.color = grid;
      if (
        usersChart.options &&
        usersChart.options.scales &&
        usersChart.options.scales.y &&
        usersChart.options.scales.y.grid
      )
        usersChart.options.scales.y.grid.color = grid;
      usersChart.update();
    }
    if (auditChart) {
      ...
      if (
        auditChart.options &&
        auditChart.options.plugins &&
        auditChart.options.plugins.legend &&
        auditChart.options.plugins.legend.labels
      )
        auditChart.options.plugins.legend.labels.color = text;
      if (
        auditChart.options &&
        auditChart.options.scales &&
        auditChart.options.scales.x &&
        auditChart.options.scales.x.ticks
      )
        auditChart.options.scales.x.ticks.color = text;
      if (
        auditChart.options &&
        auditChart.options.scales &&
        auditChart.options.scales.y &&
        auditChart.options.scales.y.ticks
      )
        auditChart.options.scales.y.ticks.color = text;
      if (
        auditChart.options &&
        auditChart.options.scales &&
        auditChart.options.scales.x &&
        auditChart.options.scales.x.grid
      )
        auditChart.options.scales.x.grid.color = grid;
      if (
        auditChart.options &&
        auditChart.options.scales &&
        auditChart.options.scales.y &&
        auditChart.options.scales.y.grid
      )
        auditChart.options.scales.y.grid.color = grid;
      auditChart.update();
    }
    if (rolesChart) {
      ...
      if (
        rolesChart.options &&
        rolesChart.options.plugins &&
        rolesChart.options.plugins.legend &&
        rolesChart.options.plugins.legend.labels
      )
        rolesChart.options.plugins.legend.labels.color = text;
      rolesChart.update();
    }
  }
