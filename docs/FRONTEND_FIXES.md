...
### 2. Прямые вызовы `console` вместо централизованного Logger ✅ Выполнено
...
### 3. Пустые catch-блоки без логирования ✅ Выполнено
...
### 8. Поле поиска `#qpSearchInput` без доступной метки ✅ Выполнено
...
### 9. Контекстные подсказки отключены в app-init ✅ Выполнено
...
**Решение:** Удалён модуль `contextual-hints.js`, так как функциональность дублируется с `OnboardingTour`, который более полноценный и интерактивный. Также удалены CSS стили для contextual-hint из `common.css`.
...
### 12. Организация RMK.css ✅ Выполнено

**Файл:** `src/css/RMK.css`

**Проблема:** Файл очень большой (~9237 строк), сложно ориентироваться и поддерживать.

**Решение:** Разбить на части:
- `rmk-base.css` — :root, темы, z-index ✅ Создан
- `rmk-layout.css` — header, sidebar, main ✅ Создан (~1962 строки)
- `rmk-radar.css` — стили радара ✅ Создан (~1426 строк)
- `rmk-modals.css` — модальные окна и формы ✅ Создан (~1894 строки)
- `rmk-components.css` — кнопки, селекты, уведомления, адаптивность ✅ Создан (~3950 строк)

Подключать все через `@import` в основной `RMK.css` или по отдельности в HTML.

**Статус:** Файл RMK.css успешно разбит на модульные файлы. Создана резервная копия RMK.css.bak. Основной файл RMK.css теперь содержит только импорты всех модулей. Каждый модуль импортирует rmk-base.css для доступа к базовым переменным и стилям.
...
| 0.3 | Составить чек-лист ручной проверки (радар, фильтры, формы, экспорт) | docs/REGRESSION_CHECKLIST.md (уже есть) |
...
**Этап 1 завершён:** 20.02.2026 — П.1 (уже было), П.8, П.2, П.3. По итогам проверки добавлено исправление сброса фильтров: при нажатии «Сбросить выбор» вызывается `Filters.updateFiltersForEnterprises()`, сбрасываются `selectedEnterprise` в localStorage и активное состояние кнопок предприятий — списки блоков и функций и радар корректно отображают все данные.
...
| Шаг | Задача | Действия |
|-----|--------|----------|
| 5.1 | **П.9** ContextualHints | ✅ Удалён модуль contextual-hints.js и CSS стили, так как функциональность дублируется с OnboardingTour. |
| 5.2 | **П.12** Разбить RMK.css | ✅ Выполнено: созданы все модульные файлы (rmk-base.css, rmk-layout.css, rmk-radar.css, rmk-modals.css, rmk-components.css). RMK.css обновлен для импорта всех модулей. Создана резервная копия RMK.css.bak. |
