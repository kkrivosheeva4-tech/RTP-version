...
**Назначение:** редирект на основную страницу радара `RMK-director.html` (для обратной совместимости).

**Что содержит:**
- Автоматический редирект на `RMK-director.html` через meta refresh и JavaScript
- Весь контент страницы удален, оставлен только редирект

**Примечание:** Все пользователи (архитекторы, директоры, РП, администраторы) теперь работают на единой странице `RMK-director.html`.
...
**Как работает:**
- Использует `RMK-director.js` для загрузки модулей
- `RMK-director.js` загружает все необходимые модули (без `prospects-chart.js`)
...
- Все роли (архитекторы, директоры, РП, администраторы) имеют доступ к этой странице
