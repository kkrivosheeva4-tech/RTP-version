...
**Назначение:** контент «О проекте».

**Важно:** это **HTML‑фрагмент**, а не полноценный документ (нет `<!doctype>`, `<head>`, `<body>`).

**Что содержит:**

- секцию `<section class="about-content">` с описанием проекта, возможностей и легенды (сектора/кольца/типы точек).

**Текущее использование в проекте:**

- прямых ссылок на `about.html` в `src/pages/*.html` нет;
- вероятно, файл используется как готовый блок для вставки/переиспользования (или оставлен «в запас»).
