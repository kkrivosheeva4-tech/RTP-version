...
1. **Модули загружаются через ES modules (Vite).**
   Точка входа — `src/main.js`, который статически импортирует все модули. API по-прежнему публикуется в `window.*` для обратной совместимости с legacy-кодом.

2. **Импорты и сборка.**
   Модули используют `import`/`export`. Сборка выполняется через `npm run build` (Vite). Пути импорта — относительно `src/main.js` (например, `./js/modules/core/logger.js`).
...
Страницы радара (`radar.html`, `index.html`) подключают **один** скрипт: `<script type="module" src="/src/main.js">`. Модули загружаются статическими импортами в `main.js`.

Высокоуровневый порядок загрузки (см. `src/main.js`):
