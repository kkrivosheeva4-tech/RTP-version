...
**Обновлено:** 04.03.2026 (документ переведен в архивный статус; актуальные техдокументы: MODULES/HTML/CSS/MATHEMATICAL)  
**Версия:** 2.2  
...
- **Этап 9. Рефакторинг для замены mock на API:** все 6 шагов завершены (9.1–9.6). Флаг USE_API, DataService, переключение mock/API, data-loader и form-management через DataService, маппинг форматов, docs/BACKEND_API_SPEC.md. state-utils: подписка на technologies использует DataService.saveTechnologies вместо vfsWrite.
...
- **Шаг 10.6. Документация:** docs/BACKEND_API_SPEC.md — порядок включения API, авторизация и токены, refresh, обработка ошибок, endpoints и форматы запросов/ответов.
...
**Статус:** ✅ Выполнено (25.02.2026). DataService использует normalizeTechnologyFromNewFormat в apiLoadTechnologies, apiCreateTech, apiUpdateTech. Создан docs/BACKEND_API_SPEC.md. JSDoc в data-normalize.js. Добавлен saveTechnologies, все vfsWrite(technologies) в data-loader и state-utils заменены на DataService.saveTechnologies.
...
**Файлы:** `src/data/ru/enterprises.json` (или отдельный `enterprise-blocks.json`), описание в `docs/BACKEND_API_SPEC.md`
...
**Файл:** `docs/BACKEND_API_SPEC.md` (новый)
...
**Статус:** ✅ Выполнено (26.02.2026). Создан docs/BACKEND_API_SPEC.md: порядок включения API (api-config.local.js, API_BASE_URL, USE_API), авторизация и токены, refresh endpoint, обработка ошибок, форматы запросов/ответов для technologies и references, маппинг (ссылка на BACKEND_API_SPEC.md), CORS и proxy.
...
- Проверить, что все endpoints задокументированы в `BACKEND_API_SPEC.md`
- Создать список требований к backend API (если ещё не создан)

**Критерий приёмки:** Frontend готов к подключению реального API.

**Статус:** ✅ Выполнено (26.02.2026). api-client и data-service проверены. BACKEND_API_SPEC.md дополнен секцией auth/2FA. Создан BACKEND_API_SPEC.md. Этап 12 завершён полностью.
...
| `scripts/ensure-api-config-local.mjs` | Создан: перед build копирует example в api-config.local.js при отсутствии файла. |
| `package.json` | Скрипт build: добавлен prebuild `node scripts/ensure-api-config-local.mjs &&`. |
...
| Файл | Изменение |
|------|-----------|
| `docs/BACKEND_API_SPEC.md` | Создан: порядок включения API (api-config.local.js, API_BASE_URL, USE_API), авторизация и токены, refresh endpoint, обработка ошибок, форматы запросов/ответов для technologies и references, маппинг (ссылка на BACKEND_API_SPEC.md), CORS и proxy. |
...
| Файл | Изменение |
|------|-----------|
| `README.md` | Добавлены: 2FA в описание авторизации; команды test/test:e2e; ссылка на BACKEND_API_SPEC.md; инструкция по api-config.local.js; страницы auth-2fa-*.html в структуру; версия РТП-3, дата 26.02.2026. |
...
| Файл | Изменение |
|------|-----------|
| `scripts/ensure-api-config-local.mjs` | Удалён; создан ensure-api-config-local.mjs для устранения предупреждения Node MODULE_TYPELESS_PACKAGE_JSON. |
...
| Файл | Изменение |
|------|-----------|
| `docs/BACKEND_API_SPEC.md` | Добавлена секция 5.1 «Аутентификация»: endpoints login, refresh, logout, 2fa/setup, 2fa/verify с форматами запросов/ответов. |
| `docs/BACKEND_API_SPEC.md` | Создан: чек-лист обязательных endpoints, форматы, CORS, ссылки на API_INTEGRATION и API_FORMAT_MAPPING. |
